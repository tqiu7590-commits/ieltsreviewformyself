  ...
  useEffect(() => {
    try {
      ...
      if (saved) {
        ...
        if (parsed.readings?.length) setSelectedReadingId(parsed.readings[0].id);
      }
...
function ReadingsPage(props) {
  ...
  const readingRetakePlan = (data.retakePlans || []).find((item) => item.readingId === selectedReadingId) || null;
    ...
    <div className="readings-layout">
      ...
      <div className="content-stack">
        {!selectedReading ? (
          ...
          <ReadingMistakeCategoryPanel
  mistakes={readingMistakes}
  selectedReading={selectedReading}
  retakePlan={readingRetakePlan}
  saveRetakePlan={saveRetakePlan}
/>
            <div className="three-col-grid">
...
function VocabularyPage({ data, searchWord, setSearchWord, statusFilter, setStatusFilter, updateWordStatus, deleteWord }) {
  ...
  return (
    <div className="page-stack">
      <Card>
        <SectionTitle icon={Library} title="本地词库" subtitle="词义只存一次，来源可以有很多条。" />
            ...
            return (
              <Card key={word.id}>
                <div className="word-head">
                  ...
                  <button onClick={() => deleteWord(word.id)} className="icon-button danger"><Trash2 size={16} /></button>
                </div>
                <p className="word-meaning">{word.meaning || "未填写中文词义"}</p>
                {word.englishMeaning && <p className="word-english">{word.englishMeaning}</p>}
                <div className="badge-row">
                  <span className="badge">出现 {occurrences.length} 次</span>
                  {caused && <span className="badge danger">曾导致错题</span>}
                </div>
                <div className="status-box">
                  <Select label="掌握状态" value={word.status} onChange={(v) => updateWordStatus(word.id, v)} options={wordStatusOptions} />
                </div>
                <div className="source-list">
                  <p className="field-label">来源</p>
                  ...
                  {occurrences.length > 3 && <p className="small-muted">还有 {occurrences.length - 3} 条来源记录。</p>}
                </div>
              </Card>
            );
          })}
